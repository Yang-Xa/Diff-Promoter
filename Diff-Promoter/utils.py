...
class GaussianDiffusion:
    ...
    def sample(self, model, image_size, batch_size=8, channels=3):
        return self.p_sample_loop(model, shape=(batch_size, channels, image_size))
...
def generate_gen(model, gaussian_diffusion, N):
    hot_one = {0: 'A', 1: 'C', 2: 'G', 3: 'T'}
    genSeq = []
    for i in range(int(N / 5000) + 1):
        m_gen = gaussian_diffusion.sample(model, 176, 5000, 4)
        for seq in m_gen[-1]:
            res = ''
            for a in seq.T[3:-3]:
                idx = np.argmax(a)
...
def getTATADistribution(real_seqs, gen_seqs):
    TATA_box = ['TATA' + a + 'A' + b for a in ['A', 'T'] for b in ['A', 'T']]
    real_rate, gene_rate = [0 for i in range(164)], [0 for i in range(164)]

    for seq in real_seqs:
        for idx in range(len(seq) - 7):
            if seq[idx: idx+7] in TATA_box:
                real_rate[idx] += 1
